...
def init_landmarks(init_measure, init_measure_cov, init_pose, init_pose_cov):
    ...
    thetat=warp2pi(init_pose[2,0])
    landmark_m = init_measure.reshape((2,k),order='F')
    betaThetaSum = landmark_m[0,:]+thetat*np.ones((1,k))
    landmark_xy=np.vstack((np.cos(betaThetaSum),np.sin(betaThetaSum)))
    r_m =np.tile(landmark_m[1,:],(2,1))
    xy = np.tile(init_pose[0:2,:],(1,k))
    landmark_2 = r_m*landmark_xy+xy
    landmark=landmark_2.reshape((2*k,1),order='F')


    landmark_cov=np.eye(2*k)
...
def predict(X, P, control, control_cov, k):
    ...
    \param control Control signal of shape (2, 1) in the polar space that moves the robot. d,alpha
    ...
    X_pre = np.copy(X)
    X_pre[0:3,:]+=np.array([[d*np.cos(thetat)],[d*np.sin(thetat)],[alpha]])

    # compute Gt
    G = np.eye(3+2*k)
    G[0:2,2:3]=np.array([[-d*np.sin(thetat)],[d*np.cos(thetat)]])
    # compute Rt
    R = np.zeros((3+2*k,3+2*k))
    A=  np.array([[np.cos(thetat),-np.sin(thetat),0],[np.sin(thetat),np.cos(thetat),0],[0,0,1]])
    R[0:3,0:3]=A@control_cov@np.transpose(A)

    P_pre = G@P@np.transpose(G)+R
...
def update(X_pre, P_pre, measure, measure_cov, k):
    ...
    for i in range(k):
        ...
        measure_i=measure[2*i:2*i+2].reshape(2,1)
        ...
        # if dx < 0:
        #     angle = warp2pi(np.arctan(dy / dx) + np.pi)
        # else:
        #     angle = warp2pi(np.arctan(dy / dx))
        # zi_bar = np.array([warp2pi(angle-thetat),qrt],dtype='float')
        zi_bar = np.array([warp2pi(np.arctan2(dy,dx)-thetat), qrt], dtype='float')

        Hi = np.array([[dy,-dx,-q,-dy,dx],[-dx*qrt,-dy*qrt,0,dx*qrt,dy*qrt]],dtype='float')/q
        Fi = np.zeros((5,3+2*k))
        Fi[0:3,0:3]=np.eye(3)
        Fi[3:5,3+2*i:3+2*i+2]=np.eye(2)
        H = Hi@Fi
        Ki = P_pre@np.transpose(H)@np.linalg.inv(H @ P_pre  @ np.transpose(H) + measure_cov)

        error_i = measure_i - zi_bar
        error_i[0,:]=warp2pi(error_i[0,:])

        X_pre = X_pre + Ki @ error_i
        P_pre = (np.eye(3+2*k)-Ki@H)@ P_pre
...
def evaluate(X, P, k):
    ...
    for i in range(k):
        l_est = X[3+2*i:3+2*i+2,0:1]
        l_real =  np.expand_dims(l_true[2*i:2*i+2], axis=1)
        dif = l_est - l_real


        euc_dis = np.sqrt(dif.T@dif)
        mah_dis = np.sqrt(dif.T@np.linalg.inv(P[3+2*i:3+2*i+2,3+2*i:3+2*i+2])@dif)
        print("For landmark %i,"%i,"euclidean=%f," %euc_dis[0,0],"mahalanobis=%f."%mah_dis[0,0])
