...
def create_linear_system(odoms, observations, sigma_odom, sigma_observation,
    ...
    for k in range(n_obs):
        ...
        b_obs = np.transpose(observations[k,2:4])
        A[l_start_ind+2*k:l_start_ind+2*k+2,2*i:2*i+2]=sqrt_inv_obs@-np.eye(2)
        A[l_start_ind + 2 * k:l_start_ind + 2 * k + 2, 2*n_poses+2 * j:2*n_poses+2 * j + 2] = sqrt_inv_obs @ np.eye(2)
        b[l_start_ind+2*k:l_start_ind+2*k+2]=sqrt_inv_obs @ b_obs

    print("end config A and b")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--method',
        nargs='+',
        choices=['default', 'pinv', 'qr', 'lu', 'qr_colamd', 'lu_colamd','lu_colamd_hand'],
        default=['pinv'],
        help='method')
    parser.add_argument(
        '--repeats',
    ...
    for method in args.method:
        ...
        #plot_traj_and_landmarks_withtitle(traj, landmarks, gt_traj, gt_landmarks,method)
